...
def cmp_fft (fft_n_turns,data):
    ...
    ##xopt_x,it_x,amp=naff(peak_frequency_x,original_signal_x) 
    xopt_x,amp,ierr,numfunc=naff(peak_frequency_x,original_signal_x) 
    ...
    return peak_frequency_x,xopt_x,amp #peak_frequency_y,xopt_x,xopt_y
...
def expected_value(a,b):
    ...
    #expected_value=np.sum([np.conj(a)*b])/len(a)
...
def naff(peak_frequency,data):
     ...
     plt.plot(x,yy)
     plt.show()
     return fminbound(y,a,b,xtol=1e-8,full_output=True)
